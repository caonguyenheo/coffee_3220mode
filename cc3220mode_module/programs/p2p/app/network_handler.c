                          ...
                          SlWlanSecParams_t SecurityParams)
{
    ...
    while(!(IS_CONNECTED(g_ulStatus)) || !(IS_IP_ACQUIRED(g_ulStatus)))
    {
        ...
        SecurityParams.Type = 2; //SL_WLAN_SEC_TYPE_OPEN
    ...
    UART_PRINT("Gateway is %d.%d.%d.%d \n\r\n\r",
               SL_IPV4_BYTE(ulDefGateway, 3), SL_IPV4_BYTE(ulDefGateway, 2), SL_IPV4_BYTE(ulDefGateway,
                                                                          1),
...
int Network_IF_provisionAP()
{
	...
	for(j=0;j<7;j++)//j loop for number of rescan
	{
