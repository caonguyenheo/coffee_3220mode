...
char g_cBsdBuf1[BUFF_LEN_FILE]= {0};
int audio_mode = -1;
//extern char buff_In[];
...
//	char acCmdStore[512];
    ...
    RetVal = sl_Stop(0xFF);
    RetVal = Network_IF_InitDriver(ROLE_AP);
...
//	int32_t             iCounter;
	...
	fd = sl_FsOpen((unsigned char *)FILENAME_MY, (SL_FS_CREATE | SL_FS_CREATE_FAILSAFE | SL_FS_CREATE_MAX_SIZE(MAX_FILE_SIZE_NEW)), 0);
	if(fd < 0)
	{
...
//    }
	...
	else
	{
		...
		{
			iStatus = sl_Recv(iNewSockID, &g_cBsdBuf1, MAX_BUF_SIZE,0);
			if((iStatus == SL_ERROR_BSD_EAGAIN) && (nb == TRUE))
			{
				sleep(1);
				continue;
