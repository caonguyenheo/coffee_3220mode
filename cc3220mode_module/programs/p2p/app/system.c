...
//			iStatus = sl_Send(iSockID, &g_cBsdBuf, buflen, 0);
			...
			UART_PRINT("waiting send [%d]...\n\r",iStatus);
			//SL_ERROR_BSD_ECONNREFUSED  SL_ERROR_BSD_EAGAIN //
//			if((iStatus == SL_ERROR_BSD_ECONNREFUSED) && (nb == TRUE))
			if(((iStatus == SL_ERROR_BSD_SOC_ERROR)|| iStatus == SL_ERROR_BSD_ECONNREFUSED) && (nb == TRUE))
			{
				UART_PRINT("waiting ...\n\r");
//		    	iStatus = sl_Connect(iSockID, sa, iAddrSize);
//		    	UART_PRINT("iStatus %d\n\r", iStatus);
