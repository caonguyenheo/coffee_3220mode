...
char g_cBsdBuf[BUF_LEN]= {0};
char gcBsdBuf_File_Rx[BUFF_LEN_FILE]= {0};
char g_cBsdBuf1[BUF_LEN]= {0};
int audio_mode = -1;
//extern char buff_In[];
...
int TcpServer(unsigned short  usPort, int nb, int tx)
{
	...
	int32_t            Status = -1;
    // filling the buffer
    memset(g_cBsdBuf1, 0x0, sizeof(g_cBsdBuf1));
    for (iCounter=0 ; iCounter<BUF_LEN ; iCounter++)
    {
    	g_cBsdBuf1[iCounter] = (char)(iCounter % 10);
    }
//    memset(&app_CB.gDataBuffer.nwData, 0x0, sizeof(app_CB.gDataBuffer));
//
...
//        iStatus = sl_SetSockOpt(iSockID, SL_SOL_SOCKET, SL_SO_KEEPALIVE,( _u8*) &enableOption, sizeof(enableOption));
	...
	if(tx == 1)
	{
		...
		iStatus = sl_Send(iSockID, &g_cBsdBuf1, buflen, 0);
	...
	else
	{
		...
		{
			iStatus = sl_Recv(iNewSockID, &g_cBsdBuf1, sizeof(g_cBsdBuf1),0);
			if((iStatus == SL_ERROR_BSD_EAGAIN) && (nb == TRUE))
			{
				sleep(1);
				continue;
		...
		UART_PRINT("Received %u packets (%u bytes) (data:%s) successfully \n\r",
				   (rcvd_bytes / BUF_LEN), rcvd_bytes, &g_cBsdBuf1);
		memset(g_cBsdBuf1, 0x0, sizeof(g_cBsdBuf1));
...
//	SlFdSet_t writesds;
	int32_t            Status = -1;
	int32_t  client_socket[30] = {0};
	int32_t  max_sd = 0;
	int32_t  sd = 0;
	int32_t  max_client = 10;
...
//    	lNonBlocking = FALSE;
    ...
    while(1)
    {
		...
		SL_SOCKET_FD_ZERO(&readsds);
		SL_SOCKET_FD_SET(iSockID, &readsds);
		max_sd = iSockID;

       //add child sockets to set
		for ( iCounter = 0 ; iCounter < max_client ;iCounter++)
		{
			//socket descriptor
			sd = client_socket[iCounter];
		...
		Status = sl_Select(max_sd + 1, &readsds, NULL, NULL, &timeout);
		if(Status < 0)
		{
			Report("Connection readsds [%d] Status[%d] \n\r", readsds, Status);
		}
		if(SL_SOCKET_FD_ISSET(iSockID, &readsds))
		{
			iNewSockID = sl_Accept(iSockID, csa, (SlSocklen_t*)&iAddrSize);
			UART_PRINT("iNewSockID:%d\n\r",iNewSockID);
	        if( iNewSockID < 0 )
	        {
	            // error
	        	ASSERT_ON_ERROR(iNewSockID, SL_SOCKET_ERROR);
	            return(-1);
	        }
	        ...
	        PrintIPAddress(0,(void*)&sAddr.in4.sin_addr);
	        UART_PRINT("\n\r");
			UART_PRINT("New connection , socket fd is %d \n\r" , iNewSockID);
			if( sl_Send(iNewSockID, "ACK", strlen("ACK"), 0) != strlen("ACK"))
			{
				UART_PRINT("[line:%d], [error:%d], [function:%s], [%s]\n\r", __LINE__, iNewSockID, __FUNCTION__, SL_SOCKET_ERROR);
			}

//			Status = sl_Select(iNewSockID, &readsds, NULL, NULL, &timeout);
			int t = 5;
	        while(t > 0)
	        {

	        	Status = sl_Recv(iNewSockID, &g_cBsdBuf1, 1480,0);
	        	Status = sl_Send(iNewSockID, "ACK\n\r", strlen("ACK\n\r"), 0);
	        	t --;
	        	if(t == 0)
	        	{
	        		UART_PRINT("Not Found Data\n\r");
	        		Status = 0;
	        		break;
	        	}
	        	UART_PRINT("[data:%s] successfully", &g_cBsdBuf1);
	        	UART_PRINT("time %d\n\r", t);

	        }

//				Status = sl_Send(iNewSockID, "ACK", strlen("ACK"), 0);
//				_ASSERT_ON_ERROR(Status);
//				UART_PRINT("\n\r");
			...
			for (iCounter = 0; iCounter < max_client; iCounter++)
			{
				...
				if( client_socket[iCounter] == 0 )
				{
					client_socket[iCounter] = iNewSockID;
...
/*		for (iCounter = 0; iCounter < max_client; iCounter++)
		{
			sd = client_socket[iCounter];
			if (SL_SOCKET_FD_ISSET( iNewSockID , &readsds))
			{
				iStatus = sl_Recv(iNewSockID, &g_cBsdBuf1, sizeof(g_cBsdBuf1),0);
				if(iStatus < 0)
				{
					UART_PRINT("Disconnect with\n\r");
					PrintIPAddress(0,(void*)&sAddr.in4.sin_addr);
					sl_Close(sd);
					client_socket[iCounter] = 0;
					continue;
				}
				else if(iStatus == 0)
				{
				...
				{
					UART_PRINT("[data:%s] successfully \n\r", &g_cBsdBuf1);
					memset(g_cBsdBuf1, 0x0, sizeof(g_cBsdBuf1));
				}
			}
		}*/
...
/*int TcpServer_multithread(unsigned short  usPort, int nb, int tx)
{
	int iCounter = 0;
	int iAddrSize = 0;
    int iSockID = 0;
...
//    iStatus = -1;
	iStatus = sl_Recv(iSockID, &g_cBsdBuf1, sizeof(g_cBsdBuf1), 0);
	_ASSERT_ON_ERROR(iStatus);
	UART_PRINT("Re [data:%s] successfully\n\r", g_cBsdBuf1);
	iStatus = sl_Send(iSockID, &g_cBsdBuf, sizeof(g_cBsdBuf), 0);
	_ASSERT_ON_ERROR(iStatus);
    ...
    while(1)
    {
		...
		else if(!strcmp(acCmdStore,"co"))
		{
			iStatus = sl_Connect(iSockID, sa, iAddrSize);
			_ASSERT_ON_ERROR(iStatus);
		}
		else if(!strcmp(acCmdStore,"end"))
		{
			break;
		}
		...
		iStatus = sl_Recv(iSockID, &g_cBsdBuf1, sizeof(g_cBsdBuf1), 0);
		_ASSERT_ON_ERROR(iStatus);
		UART_PRINT("Recv [data:%s] successfully\n\r", g_cBsdBuf1);
		UART_PRINT("\n\r");
		memset(acCmdStore, 0x0, sizeof(acCmdStore));
		memset(g_cBsdBuf1, 0x0, sizeof(g_cBsdBuf1));
    }
