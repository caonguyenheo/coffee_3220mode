...
//    	lNonBlocking = TRUE;
    	lNonBlocking = FALSE;
    	SlSockNonblocking_t BlockingOption;
    	SlSockReuseaddr_t  ReuseAddr;
    	ReuseAddr.ReuseaddrEnabled = 1;
    	BlockingOption.NonBlockingEnabled = 0;
    ...
    while(1)
    {
       ...
       //add child sockets to set
		...
		if(SL_SOCKET_FD_ISSET(iSockID, &readsds))
		{
			...
			if( sl_Send(iNewSockID, "ECHO Daemon v1.0 \r\n", strlen("ECHO Daemon v1.0 \r\n"), 0) != strlen("ECHO Daemon v1.0 \r\n"))
			{
				UART_PRINT("[line:%d], [error:%d], [function:%s], [%s]\n\r", __LINE__, iNewSockID, __FUNCTION__, SL_SOCKET_ERROR);
			}

			Status  = sl_Recv(iNewSockID, &g_cBsdBuf1, sizeof(g_cBsdBuf1),0);
			UART_PRINT("(data:%s) successfully \n\r", &g_cBsdBuf1);
		    sl_Send(iNewSockID, "ACK", strlen("ACK"), 0);
		...
		{
			sd = client_socket[iCounter];
			if (SL_SOCKET_FD_ISSET( sd , &readsds))
			{
				iStatus = sl_Recv(sd, &g_cBsdBuf1, sizeof(g_cBsdBuf1),0);
				if(iStatus == 0)
				{
...
//        iStatus = sl_SetSockOpt(iSockID, SL_SOL_SOCKET, SL_SO_KEEPALIVETIME,( _u8*) &TimeOut, sizeof(TimeOut));
    ...
    if(tx == 1)
    {
		iStatus = sl_Send(iSockID, &g_cBsdBuf, 15, 0);
		_ASSERT_ON_ERROR(iStatus);
		if(iStatus < 0)
		{
			UART_PRINT("[line:%d], [error:%d], [function:%s], [%s]\n\r", __LINE__, iStatus, __FUNCTION__, SL_SOCKET_ERROR);
