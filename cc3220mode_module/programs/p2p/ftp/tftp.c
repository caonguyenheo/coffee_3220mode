...
static int tftpSocketSetup( TFTP *pTftp )
{
    ...
    // Create UDP socket
    pTftp->Sock = sl_Socket(SL_AF_INET, SL_SOCK_DGRAM, SL_IPPROTO_UDP);
    if( pTftp->Sock < 0 )
    {
        rc = TFTPERROR_SOCKET;
        goto ABORT;
    ...
    bzero( &pTftp->tmpaddr, sizeof(SlSockAddrIn_t) );
    pTftp->tmpaddr.sin_family  = SL_AF_INET;

    // Assign local name to the unnamed socket
    // Do not care about local Host Address or Port (both 0)
    // If no error, bind returns 0
    if( sl_Bind( pTftp->Sock, (PSA)&pTftp->tmpaddr, sizeof(pTftp->tmpaddr) ) < 0 )
    {
        rc = TFTPERROR_SOCKET;
        goto ABORT;
    }
    ...
    timeout.tv_usec = 0;
    if( sl_SetSockOpt( pTftp->Sock, SL_SOL_SOCKET, SL_SO_RCVTIMEO,
    ...
    bzero( &pTftp->peeraddr, sizeof(SlSockAddrIn_t) );
    pTftp->peeraddr.sin_family      = SL_AF_INET;
    pTftp->peeraddr.sin_addr.s_addr = sl_Htonl(pTftp->PeerAddress);
    pTftp->peeraddr.sin_port        = sl_Htons(PORT_TFTP);
    rc = 0;
...
static void tftpRRQBuild(TFTP *pTftp)
{
    ...
    // Opcode = RRQ
    RRQ_Packet->opcode = sl_Htons(TFTP_RRQ);
...
static void tftpWRQBuild(TFTP *pTftp)
{
    ...
    /* Opcode = WRQ */
    WRQ_Packet->opcode = sl_Htons(TFTP_WRQ);
...
static void tftpACKBuild(TFTP *pTftp)
{
    ...
    ACK_Packet->opcode = sl_Htons(TFTP_ACK);

    // Build the rest of the ACK packet

    // Block Number
    ACK_Packet->block = sl_Htons( pTftp->NextBlock );
...
static int tftpSend( TFTP *pTftp)
{
    ...
    BytesSent = sl_SendTo(pTftp->Sock, pTftp->PacketBuffer,
                       (int)pTftp->Length, 0,(SlSockAddr_t *)&pTftp->peeraddr,
...
RETRY:
    ...
    BytesRead  = sl_RecvFrom( pTftp->Sock, ReadBuffer, DATA_SIZE, 0,
                           (SlSockAddr_t *)&pTftp->tmpaddr, (SlSocklen_t *)&addrLength );
    ...
    if( BytesRead < 0 )
    {
        ...
        // Else report error
        if( BytesRead == SL_ERROR_BSD_EWOULDBLOCK )
            BytesRead = 0;
        else
            rc = TFTPERROR_SOCKET;
        goto ABORT;
    ...
    // peer.
    if( pTftp->peeraddr.sin_port != sl_Htons(PORT_TFTP) )
    {
...
static void tftpFlushPackets(TFTP *pTftp)
{
    ...
    do
    {
        bytesread = sl_Recv( pTftp->Sock, pTftp->PacketBuffer,
                          DATA_SIZE, SL_MSG_DONTWAIT);
    } while( bytesread > 0 );
}
...
int tftpReSync( TFTP *pTftp )
{
	...
	OpCode = (unsigned short) sl_Ntohs(ReadBuffer->opcode);
		switch (OpCode)
		{
		case TFTP_DATA:
...
static int tftpProcessPacket( TFTP *pTftp )
{
    ...
    case TFTP_ERROR :
        // Copy the error code
        pTftp->ErrorCode = (unsigned short) sl_Ntohs(ReadBuffer->block);
   ...
   case TFTP_DATA :
        ...
        ServerBlock = (unsigned short) sl_Ntohs(ReadBuffer->block);
